...
```{r}
# FINAL VISUALIZATION, BY AIRPORT (CONNECTED DOT PLOT)
ggplot(planesTop10Act, aes(x = MONTH, y = PASSENGERS/1000000, fill = reorder(AIRPORT, -PASSENGERS), color = reorder(AIRPORT, -PASSENGERS))) +
  geom_line() +
  ...
  theme(plot.title = element_text(face = "bold")) +
  theme(axis.text.x = element_text(angle=45, vjust = 0.5, size = 10)) +
  labs(fill = "Airport", color = "Airport")
```

```{r}
# FINAL VISUALIZATION, BY AIRLINE (CONNECTED DOT PLOT)
